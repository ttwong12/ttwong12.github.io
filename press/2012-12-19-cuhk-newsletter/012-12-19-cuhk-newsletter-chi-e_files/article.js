...
$(document).ready(function () {
    ...
    if (document.location.hostname == "translate.itsc.cuhk.edu.hk") {
        ...
        var strSCLink = $("a#linkSC").attr("href");
        $("a#linkSC").attr("id", "linkTC").html("&#x7E41;&#x9AD4;").attr("href", strSCLink.replace("translate.itsc.cuhk.edu.hk/uniTS/", ""));
    ...
    if ($objCurrentItem != null) {
		//Hard code - remove sub-article list in article "Meeting Fresh Faces/校園新臉孔" (id:54145) in the article
		if (location.search.indexOf("articleid=54145") >= 0) {
			//Do Nothing
		} else {
    ...
    if ($objCurrentItem) {
        if ($objCurrentItem.prevAll("li").length > 0) {
            ...
            if (strLink != undefined) {
                ...
                $("<img></img>").attr("src", "/chinese/images/newsletter/news/but_prev_artic.gif").attr("alt", "Previous Article").attr("border", "0").css("display", "inline").css("padding-bottom", "5px").prependTo($link);
        ...
        if ($objCurrentItem.nextAll("li:has('>a')").length > 0) {
            ...
            if (strLink != undefined) {
                var $link = $("<a></a>").attr("href", strLink).text(strText);
                $("<img></img>").attr("src", "/chinese/images/newsletter/news/but_next_artic.gif").attr("alt", "Next Article").attr("border", "0").css("display", "inline").css("padding-bottom", "5px").prependTo($link);
                $("<br/>").insertAfter($link.find("img"));
    ...
    $("div#pageNo >a").live('click', function () {
        if ($("~ span.pageNo", $(this)).length == 0 && $("~ a", $(this)).length == 0 ) {
            ...
            //Clicked page is not the last page
            $("div#fb-comment").hide();
        }
    ...
    if (iPages > 1) {
        $("<span>頁數 </span>").appendTo("div#pageNo");
        for (i = 1; i <= iPages; i++) {
...
$(document).ready(function () {
    if ($("#fb-comment").length > 0) {
        strUrl = $("head meta[property='eng_article_url']").attr("content");
		strUrl = strUrl.replace("/cuhk-newsletter/", "/newsletter/").replace(":88/", "/");
        var strHtml = '共<fb:comments-count href="' + strUrl + '" ></fb:comments-count>則回應';
...
$(document).ready(function () {
    ...
    $("a#emailtofriend").click(function (event) {
        ...
        //Check if it is from Simplified Chinese Version
        var bSCVersion = (document.location.href.indexOf("translate.itsc.cuhk.edu.hk") >= 0);
        $.fancybox({
            'padding': 0,
            'href': 'email-to-friend.aspx' + document.location.search + (document.location.search.length == 0 ? "?" : "&") + "mode=" + strMode + (bSCVersion ? "&lang=sc" : ""),
            'width': 900,
            'height': 567,
